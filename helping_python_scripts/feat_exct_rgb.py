...
os.environ["VECLIB_MAXIMUM_THREADS"] = "1"  # Accelerate framework on macOS
# import cv2
# import numpy as np
# import onnxruntime as ort
# from tqdm import tqdm

# # Constants and PreProcess params
...
# options.intra_op_num_threads = 1  # Number of threads for parallelism within ops
...
#     # Resize the frame such that the shorter side is resize_size pixels
...
#     # Image size is 455xresize_size, take a center crop of crop_sizexcrop_size
...
#     img_array = np.transpose(img, (2, 0, 1))  # (H, W, C) -> (C, H, W)
...
# def apply_gap(features):
#     # Apply Global Average Pooling (GAP) to reduce (1, 2048, 7, 7) to (1, 2048)
#     return np.mean(features, axis=(2, 3))

# def process_frames(frame_folder, model_path, output_folder):
#     # Initialize ONNX Runtime session
#     print('Processing Video:', frame_folder)
#     ort_session = ort.InferenceSession(model_path, providers=['CPUExecutionProvider'], session_options=options)

#     # Get list of frames
#     frame_files = sorted([os.path.join(frame_folder, f) for f in os.listdir(frame_folder) if f.endswith(('.png', '.jpg', '.jpeg'))])
#     features_list = []

#     print(f"Total frames: {len(frame_files)}")


#     # Process every 6th frame starting from the 4th frame (0-based index 3)
#     for i in tqdm(range(4, len(frame_files), 6), desc="Processing frames", unit="frame"):
#         frame_path = frame_files[i]

#         # Preprocess the frame
#         input_data = preprocess_image_opencv(frame_path)

#         # Run inference
#         outputs = ort_session.run(None, {'input': input_data})

#         # Apply GAP to the output features
#         features = apply_gap(outputs[0])
#         features_list.append(features)

#     print("Total features extracted:", len(features_list))
#     # Stack features into a single array of shape (N, 2048)
#     features_array = np.vstack(features_list)

#     # Save the features as a .npy file
#     output_file = os.path.basename(frame_folder) + '.npy'
#     output_file = os.path.join(output_folder, output_file)

#     # Create the output folder if it doesn't exist
#     os.makedirs(output_folder, exist_ok=True)
#     # if os.path.exists(output_file):
#     #     # Load the vector flatten it and check cosine similarity with the new vector
#     #     old_features = np.load(output_file)
#     #     old_features = old_features.flatten()
#     #     new_features = features_array.flatten()
#     #     similarity = np.dot(old_features, new_features) / (np.linalg.norm(old_features) * np.linalg.norm(new_features))
#     #     print(f"Similarity with old features: {similarity}")
#     #     if similarity < 0.98:
#     #         printf("Aborting")
#     #         exit()
...
#     # Open a file called test_feat_flow.txt it has frame folder names in this format:
#     # frame_folder num_frames label
#     # Ignore num_frames and label and just get the frame_folder names
        
#     output_folder = "rgb_features_pruned"
#     model_path = "model.onnx"
#     # Also do the same with file test_feat_flow.txt
#     with open("test_feat_flow.txt") as f:
#         lines = f.readlines()
    
#     for line in lines:
#         frame_folder = "/data1/ghufran/test_frames/" + line.split()[0]
#         process_frames(frame_folder, model_path, output_folder)
    
#     with open("train_feat_flow.txt") as f:
#         lines = f.readlines()
...
#     for line in lines:
#         frame_folder = "/data1/ghufran/validation_frames/" + line.split()[0]
#         process_frames(frame_folder, model_path, output_folder)



#     # frame_folder = "video_validation_0000051"  # Replace with the path to your frame folder
#     # model_path = "resnet50.onnx"  # Replace with the path to your ONNX model
#     # output_folder = "rgb_features"  # Output folder to save the features
#     # process_frames(frame_folder, model_path, output_folder)
...
def process_folder(folder_path, model, out_folder, batch_size=2000):
    frame_files = sorted([f for f in os.listdir(folder_path)])

    # Keep every 6th frame starting from the 4th frame
    ...
    while count < num_frames:
        ...
        for i in range(count, min(count + batch_size, num_frames)):
            frame_path = os.path.join(folder_path, frame_files[i])
            img = Image.open(frame_path).convert('RGB')
            img = transform(img).unsqueeze(0)  # (3, H, W)
            imgs.append(img)
        
        input_batch = torch.cat(imgs, dim=0)  # (B, 3, H, W)
        # print("shape of input batch: ", input_batch.shape)


        # Add one more dime because our model needs 1, B, 3, H, W
        input_batch = input_batch.unsqueeze(0).to(device)  # (B, 1, 3, H, W)
        # exit()
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    root_dirs = [
        ...
        "/data1/ghufran/validation_frames/" + vid for vid in video_list["train_session_set"]
    ...
    root_dirs += [
        ...
        "/data1/ghufran/test_frames/" + vid for vid in video_list["test_session_set"]
    ...
    example_input = torch.randn(1, 1, 3, 224, 224).to(device)
    torch.onnx.export(model, example_input, "mobileone_rgb_deploy.onnx", export_params=True,
                      opset_version=12, do_constant_folding=True,
                      input_names=['input'], output_names=['output'])
    exit()

    # First debug 
    # root_dirs = ['/data1/ghufran/test_frames/video_test_0000897/']
    out_folder = '/data1/ghufran/helping_python_scripts/mobile_one_rgb/'
