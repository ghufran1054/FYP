validation_folder = 'validation_24FPS'
test_folder = 'test_24FPS'
train_session_set = [
...
import os
validation_folder = '/data1/ghufran/validation_frames'
test_session_set_new = []
train_session_set_new = []
# Empty the file
open('train_feat_flow.txt', 'w').close()
for video in train_session_set:
    # Open the folder validation_frames and inside that folder count the files inside the 'video' folder
    count = len([name for name in os.listdir(f'{validation_folder}/{video}')])

    # Open a file called train_feat.txt and write the video name and the count of frames
    with open('train_feat_flow.txt', 'a') as f:
        # if count % 6 != 0:
        #     count = count - (count % 6)
        if (count < 12000) :
            f.write(f'{video} {count - 1} {0}\n')
            train_session_set_new.append(video)

# Do the same for the test set
test_folder = '/data1/ghufran/test_frames'
open('test_feat_flow.txt', 'w').close()
for video in test_session_set:
    count = len([name for name in os.listdir(f'{test_folder}/{video}')])
    with open('test_feat_flow.txt', 'a') as f:
        # if count % 5 != 0:
        #     count = count - (count % 5)
        if (count < 13000):
            f.write(f'{video} {count - 1} {0}\n')
            test_session_set_new.append(video)


# # Print each element of the test_session_set_new in double quotes and inside an array just like json
print("[")
for video in test_session_set_new:
    print(f'"{video}",')
print("]")

# same for train
print("[")
for video in train_session_set_new:
    print(f'"{video}",')
print("]")